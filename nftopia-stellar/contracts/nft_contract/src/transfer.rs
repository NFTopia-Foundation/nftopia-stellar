...
use crate::events::{emit_transfer, emit_approval, emit_approval_for_all};
...
pub fn approve(env: &Env, spender: &Address, token_id: u64, sender: &Address) -> Result<(), ContractError> {
    sender.require_auth();
...
pub fn set_approval_for_all(
    ...
    env.storage()
        .instance()
        .set(&DataKey::Operator(sender.clone(), operator.clone()), &approved);
