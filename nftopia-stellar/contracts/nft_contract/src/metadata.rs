...
pub fn set_collection_config(env: &Env, config: &CollectionConfig, sender: &soroban_sdk::Address) -> Result<(), ContractError> {
    ...
    env.storage().instance().set(&DataKey::CollectionConfig, config);
    Ok(())
}
...
pub fn set_token_uri(env: &Env, token_id: u64, uri: &String, sender: &soroban_sdk::Address) -> Result<(), ContractError> {
